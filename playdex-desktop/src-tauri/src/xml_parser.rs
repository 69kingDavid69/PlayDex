...
pub async fn load_playlists_from_xml(custom_path: Option<&Path>) -> Result<Vec<Playlist>> {
    ...
    if let Some(custom_path) = custom_path {
        if !custom_path.exists() {
            bail!(XmlParseError::CustomLibraryNotFound(custom_path.display().to_string()));
        }
    }
    ...
    let path = match path {
        ...
        None => {
            ...
            if let Some(package_path) = music_library_package_path() {
                bail!(XmlParseError::MusicLibraryPackageFound(package_path.display().to_string()));
            }
    ...
    let plist = Value::from_reader(Cursor::new(data)).map_err(|_| XmlParseError::MalformedLibrary)?;

    let root_dict = plist.as_dictionary().ok_or(XmlParseError::MalformedLibrary)?;
    let tracks_dict = root_dict.get("Tracks")
        .and_then(Value::as_dictionary)
        .ok_or(XmlParseError::MalformedLibrary)?;
    let playlists_array = root_dict.get("Playlists")
        .and_then(Value::as_array)
        .ok_or(XmlParseError::MalformedLibrary)?;

    let mut playlists = Vec::new();
    for playlist_value in playlists_array {
        let playlist_dict = playlist_value.as_dictionary().ok_or(XmlParseError::MalformedLibrary)?;
        
        // Saltar playlist maestra
        if playlist_dict.get("Master").and_then(Value::as_boolean).unwrap_or(false) {
            continue;
        }

        let name = playlist_dict.get("Name")
            .and_then(Value::as_string)
            .ok_or(XmlParseError::MalformedLibrary)?
            .to_string();

        let items: &[plist::Value] = playlist_dict.get("Playlist Items")
            .and_then(Value::as_array)
        ...
        for item in items {
            let item_dict = item.as_dictionary().ok_or(XmlParseError::MalformedLibrary)?;
            let track_id_value = item_dict.get("Track ID").ok_or(XmlParseError::MalformedLibrary)?;
            ...
            let track_dict = tracks_dict.get(&track_id)
                .and_then(Value::as_dictionary)
                .ok_or(XmlParseError::MalformedLibrary)?;

            let artist = track_dict.get("Artist")
                .and_then(Value::as_string)
                .or_else(|| track_dict.get("Album Artist").and_then(Value::as_string))
                .unwrap_or("Unknown Artist")
                .to_string();
            let title = track_dict.get("Name")
                .and_then(Value::as_string)
                .unwrap_or("Unknown Title")
                .to_string();
            let album = track_dict.get("Album").and_then(Value::as_string).map(String::from);
            let isrc = track_dict.get("ISRC").and_then(Value::as_string).map(String::from);
            let duration_milliseconds = track_dict.get("Total Time")
                .and_then(|v| v.as_real().or_else(|| v.as_unsigned_integer().map(|i| i as f64)).or_else(|| v.as_signed_integer().map(|i| i as f64)))
                .unwrap_or(0.0);
...
fn music_library_package_path() -> Option<PathBuf> {
    home_dir().map(|home| home.join("Music/Music/Music Library.musiclibrary"))
        .filter(|p| p.exists())
}